...
file='./data/zemaxraytrace/tessar-offset0.txt';
...
%% Estimate circle 2
position_selection=[23 30 40]; % Choose the positions for which the top circle is unaffected by vignetting.
offaxis_distances=positions(position_selection);
pupils=pupilshapeOnRayPassPlane(1:2,position_selection,:)
...
%% Estimate circle 3
position_selection=[45 46]; % Choose the positions for which the top circle is unaffected by vignetting.
offaxis_distances=positions(position_selection);
pupils=pupilshapeOnRayPassPlane(1:2,position_selection,:);

% Tuning parameters
offset=0.2; stepsize_radius=1;
% Find radius and sensitivity of the tangent circle on the top (where it
% cuts)
...
for p=[1 10 42 43 47]
   ...
   % Hide axis but place limits
    axis off
    xlim(3*[-10 10])
    ylim(3*[-10 10]+offset(1)) %Add offset to keep circles centered in figure
...
exportgraphics(gcf,'fig/tessar-circles.pdf')
