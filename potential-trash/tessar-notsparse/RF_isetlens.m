...
filmdistance_mm=100; receptiveFieldDistanceFromFilm_mm=1383.78413
...
for i=1:numel(imageheights)
    ...
    % isetlens receptivefield
        ...
        filename=['tessar-zemax.json'];

        disp(['- Receptive field for ISETLENS: ' filename])
        
        [bins,counts,peaks]=receptiveField('isetlens',filename,filmdistance_mm,receptiveFieldDistanceFromFilm_mm,'imageheight',imageheight,'gridpoints',gridpoints,'gridsize_mm',gridsize_mm,'histogrambins',histogrambins,'diaphragmdiameter',2*8.111);
        binsLens(:,:,i,1)=bins;        countsLens(:,:,i,1)=counts;        peaksLens(:,i,1)=peaks;
...
for i=1:numel(positionIndices)
    ...
    peakshiftLens=peaksLens(xy,isel,1); % Choose one to shift peaks to be centede around 0
    ...
    for p=1:numel(degrees)
        ...
        maxValueRTF = max(countsRTF(:,xy,isel,p));
        maxValueLens = max(countsLens(:,xy,isel,1));

        

        hrtf=plot(binsRTF(:,xy,isel,p)-peakshift,countsRTF(:,xy,isel,p)/maxValueRTF,'color',color(round(25*p),:),'linewidth',1)
        hlens=plot((binsLens(:,xy,isel,p)-peakshiftLens),countsLens(:,xy,isel,p)/maxValueLens,'color',color(round(10*p),:),'linewidth',1)
...
legh=legend([hrtf hlens],'RTF','isetlens')
legh.Box='off'
