...
imageheights = [0 0.5 0.6 1 1.08]
imageheights = [0 0.5 0.6 1]
imageheights = [0 0.1 0.5 ]
imageheights = [0  ]

filmdistance_mm=2.13; receptiveFieldDistanceFromFilm_mm=18.19357+0.4
filmdistance_mm=2.13; receptiveFieldDistanceFromFilm_mm=18.51843;
%filmdistance_mm=2.1; receptiveFieldDistanceFromFilm_mm=21.29057;

pixelsize_mm=10.4e-3;
gridpoints=900
gridsize_mm=5
histogrambins=100
...
for i=1:numel(imageheights)
    ...
    for p=1:numel(degrees)
        ...
        [bins,counts,peaks]=receptiveField('rtf',filename,filmdistance_mm,receptiveFieldDistanceFromFilm_mm,'imageheight',imageheight+pixelsize_mm,'gridpoints',gridpoints,'gridsize_mm',gridsize_mm,'histogrambins',histogrambins);
...
load('zemax-lsf-2.13filmdistance+0.4mm.mat','lsf')
...
for i=1:numel(positionIndices)
    ...
    for p=1:numel(degrees)
        ...
        N=1;
        maxnorm=@(x)x/max(x)
        hzemax=plot(lsfi(:,1),maxnorm(conv(lsfi(:,2+(2-xy)),[ones(1,N)]/N,'same')),'k:','linewidth',1.5)        
